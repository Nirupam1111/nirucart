    ...
    <div className="flex flex-wrap -m-4 justify-center">
      {Object.keys(products).map((item)=>{ return <Link key={products[item]._id} passhref={true} href={`/product/${products[item].slug}`}><div className="lg:w-1/4 md:w-1/2 p-4 w-full cursor-pointer shadow-lg">
        ...
        <div className="text-center">
          ...
          <div className="mt-1">
          {products[item].size.includes('S') && <span className="px-1 border-gray-300">S </span>}
          {products[item].size.includes('M') && <span className="px-1 border-gray-300">M </span>}
          {products[item].size.includes('L') && <span className="px-1 border-gray-300">L </span>}
          {products[item].size.includes('XL') && <span className="px-1 border-gray-300">XL </span>}
          {products[item].size.includes('XXL') && <span className="px-1 border-gray-300">XLL </span>}
          </div>
          <div className="mt-1">
  ...
  let tshirts={}
    for(let item of products){
        if(item.title in tshirts){
            if(!tshirts[item.title].color.includes(item.color) && item.availablety > 0){
                tshirts[item.title].color.push(item.color)
            }
            if(!tshirts[item.title].size.includes(item.size) && item.availablety > 0){
